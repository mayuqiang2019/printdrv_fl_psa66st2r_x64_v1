...
CStateDefineRegion::CStateDefineRegion(IStateMach* pStateMach,
                                     CPrinterContext* pContext, CState* pParent) :
  CState(pStateMach, pContext, pParent)
{
  CState *pChild;

  CREATE_CHILD_STATE(CStateDeleteRegion);
  CREATE_CHILD_STATE(CStateAddRegion);
}

/// <summary>Rerieves ID of this state.</summary>
int CStateDefineRegion::GetID()
{
  return STATE_DEFINE_REGION;
}

/// <summary>Handles state entered event.</summary>
/// <param name="isTarget">True if this state is the final target of transition,
/// false otherwise.</param>
void CStateDefineRegion::OnEnter(bool isTarget)
{
  if(isTarget)
  {
    m_pContext->Trace(L"[printdrv_fl_psa66st2r][CStateDefineRegion::OnEnter]\n");
  }
}
